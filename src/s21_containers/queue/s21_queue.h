...
class Queue {
 ...
 public:
  ...
  Queue(const Queue<value_type> &q) {  // copy constructor
    if (q.empty()) {
      throw std::out_of_range("Unable to copy empty Queue into another");
    }
    Container *current_other = q.beginning;
    while (current_other != nullptr) {
      push(current_other->value);
  ...
  void clear() {
    while (number_of_elements != 0) {
      if (end != nullptr) {
        ...
        throw std::logic_error("Unable to clear Queue");
      }
    }
  }
  ...
  void pop() {  // removes the first element
    if (number_of_elements == 0) {
      throw std::out_of_range("Unable to pop from an empty Queue");
    }
